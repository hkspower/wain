...
//
// So /backends was inviting crawlers in and claiming to be the storefront's home
...
// is the only group it reads, so the `Disallow: /backends` in the wildcard group
...
export function useAdminMeta(title = 'Sporta Backends') {
  useEffect(() => {
    ...
    // The homepage canonical must not be asserted from /backends. Detached rather

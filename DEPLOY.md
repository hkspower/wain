...
| `MAWSOOL_TRUST_PROXY=1` | يجعل النظام يصدّق `X-Forwarded-For` في تحديد عنوان الطالب | **نعم** خلف وكيل عكسي |
...
| `MAWSOOL_VOICE_MAX_PER_ORDER` | سقف الملاحظات الصوتية على الطلب الواحد (افتراضيًا ١٠) | — |
| `MAWSOOL_WEBHOOK_URL` | وجهة دفع الأحداث للخارج (n8n) | لأتمتة واتساب |
| `MAWSOOL_WEBHOOK_SECRET` | سرّ توقيع الأحداث — بدونه لا يُرسل حدث | مع الوجهة |
| `MAWSOOL_PUBLIC_URL` | العنوان العام لبناء روابط المهام في الأحداث | مع الوجهة |
| `MAWSOOL_WEBHOOK_AUTH_HEADER` · `MAWSOOL_WEBHOOK_AUTH_VALUE` | ترويسة مصادقة ثابتة للوجهة (افتراضها `Authorization`) — يتحقّق منها n8n باعتماداته، فلا يُكتب السرّ في سير العمل | اختياري |
| `MAWSOOL_WEBHOOK_TIMEOUT_MS` | مهلة دفع الحدث — الوجهة الخارجية لا تعلّق طلب المدير (أدناها ١٠٠٠) | `8000` |

> **`MAWSOOL_TRUST_PROXY` ليس تزيينًا.** حدّ محاولات الدخول مفتاحه عنوان
> الطالب. بلا وكيل عكسي أمام النظام يجب أن يبقى مطفأً: `X-Forwarded-For`
> ترويسة يكتبها العميل، وتصديقها بلا وسيط يجعل تغييرها كل محاولة يمنح مفتاحًا
> جديدًا فلا يُحجب التخمين أبدًا. ومع وكيل عكسي يجب أن يكون مشتعلًا، وإلا
> بدت كل الطلبات قادمة من عنوان الوكيل نفسه فحُجب المستخدمون جميعًا معًا.
> وفي الحالتين يبقى حدٌّ ثانٍ على اسم المستخدم يقيّد التخمين الموزّع.

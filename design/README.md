...
Hand-drawn, and the design work:
...
**Generated by `npm run design:boards`, and not to be hand-edited** — the
theme a logo has to be designed against, read from `src/app/globals.css` and
`src/components/WainLogo.tsx` rather than retyped:

- `Palette.dc.html` — 52 swatches across six ramps, plus the three hairlines
- `Type.dc.html` — 11 steps with their sizes and leading, set in Arabic
- `Surfaces.dc.html` — the eight category grounds at 40px and 24px

`npm run scan` runs `design:boards:check` and fails if any of the three has
drifted from the theme. Change a token, re-run the script, re-seed the canvas.
They are generated for the same reason `gen-design-system.mjs` generates the
icon block: a specification that is a hand-copy goes on looking authoritative
after it stops being true, and this directory has already been bitten by that
once — the annotation below claimed a mark was the component's when its «؟»
had been quietly redrawn.

Two things worth knowing if you edit the generator. The frame heights in
`canvas.json` are measured from the rendered content, not guessed: the first
Palette board was declared at 700px, wanted 821, and silently lost its ink ramp
and every hairline off the bottom. And the keyline is white on every ground —
drawn once with the keyline set to the ink beneath it, the ؟ and its dot
disappeared, because the keyline is not an outline round the mark, it is what
draws the ؟.

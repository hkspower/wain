...
(() => {
   ...
   *
   * **والترتيب هنا هو ترتيب السؤال** — وهو مقصود: الوجهة قبل الهُويّة.
   * كان الاسم أوّلًا، فقِيس حوارٌ واقعيّ: قال الزبون «السلام عليكم» فردّ
   * الوكيل «ما اسمك؟»، وسأل «كم سعر التوصيل؟» فردّ «ما اسمك؟». وهذا
   * استجوابٌ لا محادثة: يُسأل الغريب عن اسمه قبل أن يقول ما يريد أصلًا.
   *
   * والزبون يفكّر بالطلب لا بنفسه: «ودّي أطرش من حولي للفحيحيل» جملةٌ
   * تأتي وحدها، والاسمُ والرقم يأتيان حين يُطلبان في آخر الكلام — كما
   * يسأل الموظّف على الهاتف. والاسم أعلى الحقول احتكاكًا أيضًا: يحتاج
   * صيغةً («اسمي فلان») ولا يُفهم مجرّدًا، فتأخيره يؤخّر أوّل تعثّر.
   *
   * ويسري الترتيب على الإقرار والنواقص معًا: «تمام — الاستلام من حولي،
   * التسليم في الفحيحيل، الاسم نورة» تُقرأ كما وقع الكلام. */
  const REQUIRED = ['pickup_area', 'dropoff_area', 'customer_name', 'customer_phone'];
  ...
  const state = {
    utterances: [],     // ما ضمّه الخادم إلى **الطلب**، بترتيبه
    /* وما قاله الزبون كلّه — بما فيه أسئلته. وهما ليسا واحدًا: السؤال
       يُجاب ولا يدخل الطلب (وذاك صواب، وإلّا صار «كم السعر؟» عنوانًا)،
       لكنّه من كلام الزبون، والمكتب يقرأ الحديث لا الحقول. وكان المُرسَل
       `utterances` فكان الوعد «حديثك كلّه وصل» أوسع من الواقع. */
    said: [],
  ...
  function nextQuestion(gained) {
    ...
    /* «العالق» من حاول أن يجيب فلم يُفهم — لا من سأل سؤالًا فأُجيب.
       قِيس حوارٌ سأل فيه الزبون عن السعر والتغطية والوقت، فقيل له بعد
       ثالث سؤال «وإن تعذّر: راسلنا على واتساب» — وهو لم يتعثّر في شيء،
       إنّما كان يستطلع قبل أن يطلب. فالسؤال لا يُعدّ تعثّرًا. */
    const asked = !!(state.parsed && (state.parsed.answer || state.parsed.unanswered));
    state.stuck = again && !gained && !asked ? (state.stuck || 1) + 1 : 1;
  ...
  async function runTurn(raw) {
    ...
    /* ما قاله الزبون يُحفظ كما قاله — بعد أن تنجح الجولة لا قبلها، فسطرٌ
       سقطت شبكتُه يُعاد إرساله فلا يُسجَّل مرّتين. */
    state.said.push(raw);
  ...
  async function applyHint(from, hintName) {
    ...
    /* والتصحيح من كلام الزبون أيضًا: من يقرأ الحديث في اللوحة يحتاج أن
       يعرف أنّ المنطقة صُحّحت، لا أن يجدها صحيحةً بلا سبب. */
    state.said.push(`نعم، أقصد ${hintName}`);
    renderCard();
    nextQuestion(acknowledge());
  }
      ...
      const res = await fetch(`${API}/api/public/order`, {
        ...
        body: JSON.stringify({
          ...
          /* ما قاله الزبون بحروفه. الحقول أعلاه ما فهمه الوكيل، وهذا ما
             ...
             و`said` لا `utterances`: الثاني ما دخل الطلب، والأوّل ما قيل. */
          transcript: state.said.slice(),
        }),
      });
  ...
  /* الصفحة بلا نصٍّ ساكن، فالسطر تحت الملتقط يبقى فارغًا في السكون.
     ولا يُترك أبدًا: الالتقاط يقول فيه إنّه يسمع، والخطأ يقول فيه سببه —

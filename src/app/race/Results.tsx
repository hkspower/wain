      ...
      >
       <div className="results-grid">
        ...
        <div className="text-center">
          ...
          <p className="grn-label mt-1.5 text-xs">
        ...
        >
          <div className="flex items-end justify-between">
            <span className="grn-label text-xs">
          ...
          <div className="mt-1 flex items-center justify-between">
            ...
            {levelUp && (
              <span className="grn-display text-xs tracking-[0.18em] text-sodium-400">
                LEVEL UP
              </span>
            )}
          </div>
          ...
          <ul className="mt-2.5 grid gap-1 sm:grid-cols-2">
            {result.xpBreakdown.map((b, i) => (
              <li
                key={b.label}
                className="reveal flex items-center justify-between rounded-md bg-white/[0.04] px-2.5 py-1 text-xs text-white/75"
        ...
        {result.rewards.length > 0 && (
          <div className={`mt-4 ${beat >= 3 ? "" : "invisible"}`}>
            <div className="grn-label mb-1.5 text-2xs">Unlocked</div>
              ...
              {result.rewards.map((r, i) => (
                ...
                >
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="block truncate text-xs text-white/55">{r.sub}</span>
                  </span>
                </div>
              ))}
            </div>
          ...
          {[
            ...
            >
              <div className="grn-display tnum text-lg leading-tight text-white">
                {s.v}
                {s.u && <span className="ml-0.5 text-2xs text-white/74">{s.u}</span>}
              </div>
              <div className="grn-label text-2xs text-white/70">{s.k}</div>
            </div>
          ))}
        </div>
        <div
          className={`mt-2 text-center text-xs text-white/70 transition-opacity duration-300 ${
            beat >= 4 ? "opacity-100" : "opacity-0"
          }`}
        >
        ...
        >
          {result.nextRival && won && (
            <p className="mb-2.5 text-center text-xs text-white/65">
              Next up:{" "}
        ...
        {beat < 5 && (
          <p className="grn-label mt-4 text-center text-2xs text-white/62">
            tap to skip
          </p>
        )}
      </div>

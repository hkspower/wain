    ...
    art: (
      <div className="flex w-full items-center justify-center gap-2 text-center">
        {["TURBO", "TYRES", "NOS", "PAINT"].map((t) => (
          <span
            key={t}
            className="grn-label rounded-md border border-white/15 bg-white/[0.06] px-2 py-1.5 text-2xs"
          >
            {t}
          </span>
        ))}
    ...
    >
      <div className="grn-dialog screen-in safe-pad w-[min(560px,94vw)] p-5 sm:p-7">
        <div className="flex items-center justify-between">
          <span className="grn-label text-2xs text-sodium-400">{c.kicker}</span>
          <button
            onClick={skip}
            className="grn-label tap px-2 text-2xs text-white/74 hover:text-white"
          >
            SKIP
          </button>
        </div>
  ...
  return (
    <div className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 translate-y-[3.2rem]">
      <div className="grn-info coach-ring reveal px-4 py-2">
        <span className="grn-display text-sm tracking-[0.08em]">{text}</span>
      </div>
    </div>
  );
}

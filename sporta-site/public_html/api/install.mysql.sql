...
--
-- A STALE COPY. It says it is generated by scripts/make-install-sql.mjs from
-- "the four part files"; that generator now builds TEN parts into
-- database-sql/IMPORT-THIS-ONE.sql instead. `npm run make:install` does not
-- write this file and has not for some time, so editing a part and rebuilding
-- leaves this one exactly as it was.
--
-- THE CURRENT BUNDLE IS database-sql/IMPORT-THIS-ONE.sql. Import that one.
-- This copy is kept because it is on the live server and named in
-- live-file-check's manifest; it is corrected rather than left to rot, but it
-- lacks the returns tables, the admin email code and the assistant Q&A table
-- that the current bundle carries.
--
-- HOW TO USE IT

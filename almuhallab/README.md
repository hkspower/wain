...
- `robots.txt` and `sitemap.xml` (the three indexable pages only — the admin
  console, the redirect stubs and 404 are `noindex` and stay out).
...
```bash
python3 design/test_suite.py     # 447 checks, exits non-zero on failure
```

...
/* سُئل «ما اسمك؟» فقال «اممم» وهو يفكّر، فصار ذلك اسمَ صاحب الطلب ومضى
   إلى الكابتن ينادي به عند الباب. والحشو ليس اسمًا وإن وقع موقع الجواب. */
test('حشو الكلام وتحيّاته ليست اسمًا', () => {
  for (const t of ['اممم', 'امم', 'اه', 'يعني', 'هلا', 'مرحبا', 'شكرا', 'لا ادري', 'ok', 'شنو']) {
    assert.equal(V.isFiller(t), true, `«${t}» لم يُعدّ حشوًا`);
  }
  /* ولا يُوسَّع الجدول على حساب أسماء حقيقية */
  for (const t of ['بدر', 'نورة', 'زين', 'أمل', 'بدر العجمي', 'أم محمد']) {
    assert.equal(V.isFiller(t), false, `«${t}» عُدّ حشوًا وهو اسم`);
  }
  assert.equal(V.isFiller(''), true);
});

/* «لا أدري» جوابٌ عن سؤال، لا اسمُ مكانٍ أُخطئ في كتابته. وكان الوكيل
   يردّ: «"أدري" ليست من مناطق الكويت — هل تقصد "الري"؟» */
test('لا يُقترح اسم منطقة على حشو الكلام، ويبقى الاقتراح على ما قاربها', () => {
  const SIM = require('../server/similar');
  const AREA = require('../server/areas');
  const skip = { skip: V.isFiller };
  assert.equal(SIM.closestInText('لا ادري', AREA.ALL_AREAS, skip), null);
  assert.equal(SIM.closestInText('لا ادري', AREA.ALL_AREAS)?.name, 'الري',
    'الاختبار بلا معنى إن لم يكن الاقتراح واقعًا أصلًا');
  assert.equal(SIM.closestInText('من الفحيحل', AREA.ALL_AREAS, skip)?.name, 'الفحيحيل');
});

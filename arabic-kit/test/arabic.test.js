...
test('حماية النص اللاتيني من انقلاب الاتجاه — بالعزل لا بعلامة الاتجاه', () => {
  /* كان يضع LRM (U+200E) وحدها، **وهي لا تفعل شيئًا هنا**. قِيس موضع «+»
     بالبكسل في صفحةٍ عربية عبر Range: بقيت في يمين الأرقام كما لو لم تكن
     العلامة — لأنّ LRM محايدةُ حدٍّ تؤثّر في ترتيب المحايدات حولها ولا
     تعزل ما بعدها. جُرّبت خمسة أساليب وقِيست، فسقطت هي ونجح العزل الصريح.
     واختير العزل بالمحارف لا بالوسوم ليعمل في الصفحة والبريد وواتساب سواء. */
  assert.equal(ar.ltr('+965 2222 0000'), '\u2066+965 2222 0000\u2069');
  assert.equal(ar.ltr('MW-4821').charCodeAt(0), 0x2066);
  assert.equal(ar.ltr('MW-4821').charCodeAt(ar.ltr('MW-4821').length - 1), 0x2069);
  /* والمحتوى بينهما كما هو، بلا زيادةٍ ولا نقص */
  assert.equal(ar.ltr('MW-4821').slice(1, -1), 'MW-4821');
});
